...
class CartRepo {
  ...
  List<TemporaryCartListModel> getCustomerCartList() {
    ...
    if (sharedPreferences.containsKey(AppConstants.CUSTOMER_CART_LIST)) {
      customerCarts =
          sharedPreferences.getStringList(AppConstants.CUSTOMER_CART_LIST);
    }
    List<TemporaryCartListModel> customerCart = [];
    customerCarts.forEach((cart) =>
        customerCart.add(TemporaryCartListModel.fromJson(jsonDecode(cart))));
    return customerCart;
  }

  void addToCartListForCustomer(List<TemporaryCartListModel> customerCartList) {
    List<String> customerCarts = [];
    customerCartList.forEach((customerCartModel) =>
        customerCarts.add(jsonEncode(customerCartModel)));
    sharedPreferences.setStringList(
        AppConstants.CUSTOMER_CART_LIST, customerCarts);
  }

  Future<Response> getCouponDiscount(
      String couponCode, int userId, double orderAmount) async {
    return await apiClient.getData(
        '${AppConstants.GET_COUPON_DISCOUNT}?code=$couponCode&user_id=$userId&order_amount=$orderAmount');
  }
  ...
  Future<Response> getProductFromScan(String productCode) async {
    return await apiClient.getData(
        '${AppConstants.GET_PRODUCT_FROM_PRODUCT_CODE}?product_code=$productCode');
  }

  Future<Response> customerSearch(String name) async {
    return await apiClient
        .getData('${AppConstants.CUSTOMER_SEARCH_URI}?name=$name');
  }
}

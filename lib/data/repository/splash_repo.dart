...
class SplashRepo {
  ...
  Future<Response> getDashboardRevenueSummery(String filterType) async {
    Response _response = await apiClient.getData(
        '${AppConstants.GET_DASHBOARD_REVENUE_SUMMERY}?statistics_type=$filterType');
    return _response;
  }
  ...
  Future<http.StreamedResponse> updateShop(
    ...
    http.MultipartRequest request = http.MultipartRequest('POST',
        Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_SHOP_URI}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    if (file != null) {
      Uint8List _list = await file.readAsBytes();
      var part = http.MultipartFile(
          'shop_logo', file.readAsBytes().asStream(), _list.length,

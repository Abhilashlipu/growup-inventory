...
class CustomerRepo {
  ...
  Future<Response> getCustomerWiseOrderList(int customerId, int offset) async {
    return await apiClient.getData(
        '${AppConstants.CUSTOMER_WISE_ORDER_LIST_URI}?customer_id=$customerId&limit=10&offset=$offset');
  }
  ...
  Future<Response> deleteCustomer(int customerId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_CUSTOMER_URI}?id=$customerId');
  }
  ...
  Future<http.StreamedResponse> addCustomer(
    ...
    http.MultipartRequest request = isUpdate
        ? http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.UPDATE_CUSTOMER_URI}'))
        : http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.ADD_NEW_CUSTOMER}'));

...
class ProductRepo {
  ...
  Future<Response> getProductList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_PRODUCT_URI}?limit=10&offset=$offset');
  }

  Future<Response> getLimitedStockProductList(int offset) async {
    return await apiClient.getData(
        '${AppConstants.GET_LIMITED_STOCK_PRODUCT_URI}?limit=10&offset=$offset');
  }

  Future<Response> updateProductQuantity(int productId, int quantity) async {
    return await apiClient.getData(
        '${AppConstants.UPDATE_PRODUCT_QUANTITY}?id=$productId&quantity=$quantity');
  }
  ...
  Future<http.StreamedResponse> addProduct(
    ...
    http.MultipartRequest request = isUpdate
        ? http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.UPDATE_PRODUCT_URI}'))
        : http.MultipartRequest('POST',
            Uri.parse('${AppConstants.BASE_URL}${AppConstants.ADD_PRODUCT}'));
    ...
    print(
        '===here is product==>${product.toJson()} $categoryId and $subCategoryId');
  ...
  Future<Response> searchProduct(String productName) async {
    return await apiClient
        .getData('${AppConstants.PRODUCT_SEARCH_URI}?name=$productName');
  }
  ...
  Future<http.StreamedResponse> bulkImport(File filePath, String token) async {
    http.MultipartRequest request = http.MultipartRequest(
        'POST',
        Uri.parse(
            '${AppConstants.BASE_URL}${AppConstants.BULK_IMPORT_PRODUCT}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    if (filePath != null) {
      Uint8List _list = await filePath.readAsBytes();
      var part = http.MultipartFile(
          'products_file', filePath.readAsBytes().asStream(), _list.length,
  ...
  Future<Response> downloadSampleFile() async {
    return await apiClient
        .getData('${AppConstants.GET_DOWNLOAD_SAMPLE_FILE_URL}');
  }

  Future<Response> deleteProduct(int productId) async {
    return await apiClient
        .getData('${AppConstants.PRODUCT_DELETE_URI}?id=$productId');
  }

  Future<Response> barCodeDownLoad(int id, int quantity) async {
    return await apiClient
        .getData('${AppConstants.BAR_CODE_DOWNLOAD}?id=$id&quantity=$quantity');
  }
}

...
class CouponRepo {
  ...
  Future<Response> getCouponList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_COUPON_LIST}?limit=10&offset=$offset');
  }

  Future<Response> addNewCoupon(Coupons coupon, {bool update = false}) async {
    return await apiClient.postData(
        update
            ? '${AppConstants.UPDATE_COUPON}'
            : '${AppConstants.ADD_NEW_COUPON}',
        {
          'id': coupon.id,
          'title': coupon.title,
  ...
  Future<Response> toggleCouponStatus(int couponId, int status) async {
    return await apiClient.getData(
        '${AppConstants.UPDATE_COUPON_STATUS}?id=$couponId&status=$status');
  }

  Future<Response> deleteCoupon(int couponId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_COUPON_URI}?id=$couponId');
  }
}

...
class SupplierRepo {
  ...
  Future<Response> getSupplierList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_SUPPLIER_LIST}?limit=10&offset=$offset');
  }

  Future<Response> getSupplierProfile(int sellerId) async {
    return await apiClient
        .getData('${AppConstants.SUPPLIER_PROFILE_URI}?id=$sellerId');
  }

  Future<Response> getSupplierProductList(int offset, int supplierId) async {
    return await apiClient.getData(
        '${AppConstants.SUPPLIER_PRODUCT_LIST_URI}?limit=10&offset=$offset&supplier_id=$supplierId');
  }

  Future<Response> getSupplierTransactionList(
      int offset, int supplierId) async {
    return await apiClient.getData(
        '${AppConstants.SUPPLIER_TRANSACTION_LIST_URI}?limit=10&offset=$offset&supplier_id=$supplierId');
  }

  Future<Response> getSupplierTransactionFilterList(
      int offset, int supplierId, String fromDate, String toDate) async {
    return await apiClient.getData(
        '${AppConstants.SUPPLIER_TRANSACTION_FILTER_LIST_URI}?supplier_id=$supplierId&limit=10&offset=$offset&from=$fromDate&to=$toDate');
  }
  ...
  Future<Response> supplierNewPurchase(int supplierId, double purchaseAmount,
    ...
    return await apiClient
        .postData('${AppConstants.NEW_PURCHASE_FROM_SUPPLIER}', {
  ...
  Future<Response> supplierPayment(int supplierId, double totalDueAmount,
  ...
  Future<Response> searchSupplier(String name) async {
    return await apiClient
        .getData('${AppConstants.SEARCH_SUPPLIER_URI}?name=$name');
  }
  ...
  Future<http.StreamedResponse> addSupplier(
    ...
    http.MultipartRequest request = isUpdate
        ? http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.UPDATE_SUPPLIER_URI}'))
        : http.MultipartRequest('POST',
            Uri.parse('${AppConstants.BASE_URL}${AppConstants.ADD_SUPPLIER}'));
  ...
  Future<Response> deleteSupplier(int supplierId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_SUPPLIER_URI}?id=$supplierId');
  }
}

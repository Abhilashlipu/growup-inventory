...
class AccountRepo {
  ...
  Future<Response> getAccountList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_account_LIST}?limit=10&offset=$offset');
  }

  Future<Response> searchAccount(String search) async {
    return await apiClient
        .getData('${AppConstants.SEARCH_ACCOUNT_URI}?name=$search');
  }

  Future<Response> deleteAccountId(int accountId) async {
    return await apiClient
        .getData('${AppConstants.DElETE_ACCOUNT_URI}?id=$accountId');
  }

  Future<Response> addAccount(Accounts account, {bool isUpdate = false}) async {
    return await apiClient.postData(
        isUpdate
            ? '${AppConstants.UPDATE_ACCOUNT_URI}'

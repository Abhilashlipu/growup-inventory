...
class TransactionRepo {
  ...
  Future<Response> getTransactionAccountList(int offset) async {
    return await apiClient.getData(
        '${AppConstants.TRANSACTION_ACCOUNT_LIST_URI}?limit=10&offset=$offset');
  }

  Future<Response> getCustomerWiseTransactionList(
      int customerId, int offset) async {
    return await apiClient.getData(
        '${AppConstants.CUSTOMER_WISE_TRANSACTION_LIST_URI}?customer_id=$customerId&limit=10&offset=$offset');
  }
  ...
  Future<Response> getTransactionList(int offset) async {
    return await apiClient.getData(
        '${AppConstants.TRANSACTION_LIST_URI}?limit=10&offset=$offset');
  }

  Future<Response> exportTransactionList(String startDate, String endDate,
      int accountId, String transactionType) async {
    return await apiClient.getData(
        '${AppConstants.TRANSACTION_LIST_EXPORT_URI}?from=$startDate&to=$endDate&account_id=$accountId&transaction_type=$transactionType');
  }

  Future<Response> getTransactionFilter(String startDate, String endDate,
      int accountId, String transactionType) async {
    return await apiClient.getData(
        '${AppConstants.TRANSACTION_FILTER_URI}?from=$startDate&to=$endDate&account_id=$accountId&transaction_type=$transactionType');
  }

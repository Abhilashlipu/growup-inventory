...
class BrandRepo {
  ...
  Future<Response> getBrandList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_BRAND_LIST}?limit=10&offset=$offset');
  }
  ...
  Future<http.StreamedResponse> addBrand(
    ...
    http.MultipartRequest request = isUpdate
        ? http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.UPDATE_BRAND_URI}'))
        : http.MultipartRequest('POST',
            Uri.parse('${AppConstants.BASE_URL}${AppConstants.ADD_BRAND}'));
  ...
  Future<Response> deleteBrand(int brandId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_BRAND_URI}?id=$brandId');
  }
}

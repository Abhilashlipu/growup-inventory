...
class ExpenseRepo {
  ...
  Future<Response> getExpenseList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_EXPENSE_LIST}?limit=10&offset=$offset');
  }

  Future<Response> getExpenseFilter(String startDate, String endDate) async {
    return await apiClient.getData(
        '${AppConstants.EXPENSE_FILTER_BY_DATE}?from=$startDate&to=$endDate');
  }

  Future<Response> deleteExpense(int expenseId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_EXPENSE_URI}?id=$expenseId');
  }
  ...
  Future<Response> addNewExpense(Expenses expense,
    ...
    return await apiClient.postData(
        isUpdate
            ? '${AppConstants.UPDATE_EXPENSE_URI}'
            : '${AppConstants.ADD_NEW_EXPENSE}',
        {
          'id': expense.id,

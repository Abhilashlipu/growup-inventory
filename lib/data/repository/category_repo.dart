...
class CategoryRepo {
  ...
  Future<Response> getCategoryList(int offset) async {
    return await apiClient
        .getData('${AppConstants.GET_CATEGORY_LIST}?limit=10&offset=$offset');
  }

  Future<Response> getCategoryWiseProductList(int categoryId) async {
    return await apiClient
        .getData('${AppConstants.CATEGORIES_PRODUCT}?category_id=$categoryId');
  }
  ...
  Future<http.StreamedResponse> addCategory(
    ...
    http.MultipartRequest request = isUpdate
        ? http.MultipartRequest(
            'POST',
            Uri.parse(
                '${AppConstants.BASE_URL}${AppConstants.UPDATE_CATEGORY_URI}'))
        : http.MultipartRequest('POST',
            Uri.parse('${AppConstants.BASE_URL}${AppConstants.ADD_CATEGORY}'));
  ...
  Future<Response> getSubCategoryList(int offset, int categoryId) async {
    return await apiClient.getData(
        '${AppConstants.GET_SUB_CATEGORY_LIST}?limit=10&offset=$offset&category_id=$categoryId');
  }
  ...
  Future<Response> addSubCategory(
    ...
    return await apiClient.postData(
        isUpdate
            ? '${AppConstants.UPDATE_SUB_CATEGORY}'
            : '${AppConstants.ADD_SUB_CATEGORY}',
  ...
  Future<Response> deleteCategory(int categoryId) async {
    return await apiClient
        .getData('${AppConstants.DELETE_CATEGORY_URI}?id=$categoryId');
  }

  Future<Response> categoryStatusOnOff(int categoryId, int status) async {
    return await apiClient.getData(
        '${AppConstants.UPDATE_CATEGORY_STATUS_URI}?id=$categoryId&status=$status');
  }
}
